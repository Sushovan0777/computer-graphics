...
pygame.display.set_caption("Grid Settings")
...
# Labels
labels = [
    {"text": "Rows:", "pos": (50, 110)},
    {"text": "Columns:", "pos": (50, 160)},
    {"text": "Tile Size:", "pos": (50, 210)}
]

# Title
title = {"text": "Enter Grid Settings", "pos": (50, 50)}
    ...
    try:
        ...
        if box["type"] == "rows":
            ...
            if value < 1:
                ...
                cols = value
        elif box["type"] == "tile_size":
            if value < 10:
                tile_size = 10
            elif value > 95:
                tile_size = 95
            else:
                tile_size = value // 5 * 5  # Ensure tile_size is a multiple of 5
    except ValueError:
        pass
...
def update_value(action, type_):
    global rows, cols, tile_size
    if type_ == "rows":
        if action == "increment" and rows < 18:
            rows += 1
        elif action == "decrement" and rows > 1:
            rows -= 1
    elif type_ == "cols":
        if action == "increment" and cols < 38:
            cols += 1
        elif action == "decrement" and cols > 1:
            cols -= 1
    elif type_ == "tile_size":
        if action == "increment" and tile_size < 100:
            tile_size += 5
